...
import (
	...
	"github.com/bytom/bystack/p2p/upnp"
)
...
func getUPNPExternalAddress(externalPort, internalPort int) (*NetAddress, error) {
	...
	externalPort, err = nat.AddPortMapping("tcp", externalPort, internalPort, "bystackd tcp", 0)
	if err != nil {
		return nil, errors.Wrap(err, "could not add tcp UPNP port mapping")
	}
	externalPort, err = nat.AddPortMapping("udp", externalPort, internalPort, "bystackd udp", 0)
	if err != nil {
...
//DefaultListener Implements bystackd server Listener
